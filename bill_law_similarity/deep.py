...
class BertSimilarity:
    ...
    def bert_cosine(self, bill_sents, law_sents):
        bill_emb = self.model.encode(bill_sents, convert_to_tensor=True)
        law_emb = self.model.encode(law_sents, convert_to_tensor=True)
        cosine_scores = util.pytorch_cos_sim(bill_emb, law_emb)
        return cosine_scores.to("cpu")

    def bert_align(self, bill_sents, law_sents, verbose=False):
        cosine_scores = self.bert_cosine(bill_sents, law_sents)
    ...
    def bert_avg_pool(self, bill_sents, law_sents):
        bill_emb = self.model.encode(bill_sents, convert_to_tensor=True).mean(dim=0)
        law_emb = self.model.encode(law_sents, convert_to_tensor=True).mean(dim=0)
        cosine_scores = util.pytorch_cos_sim(bill_emb, law_emb)[0]
        return cosine_scores.to("cpu").item()

    def bert_max_avg(self, bill_sents, law_sents):
        bill_emb = self.model.encode(bill_sents, convert_to_tensor=True)
        law_emb = self.model.encode(law_sents, convert_to_tensor=True)
        cosine_scores = util.pytorch_cos_sim(bill_emb, law_emb)
