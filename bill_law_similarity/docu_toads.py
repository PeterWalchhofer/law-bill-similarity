# All code taken from https://github.com/ajhhermansson/DocuToads and adapted
...
def article_data_sorter(
    ...
    artranges1 = artranger(
        artindex1, lentext1
    )  # Create ranges delineating each article with a lower and upper bound (index)
    ...
    for step in backtrace:  # Loop over each step in the backtrace
        ...
        for j, ar in enumerate(artranges1):
            if (
                ar[0] <= rowindex and rowindex <= ar[1]
            ):  # With last row, find whic article the step belongs in (text1)
                for k, ar2 in enumerate(artranges2):
                    if (
                        ar2[0] <= columnindex and columnindex <= ar2[1]
                    ):  # Same for text 2
                        ar[2].append(
                            [step[2], artnames2[k]]
                        )  # Sort the step into a specific article and save it there
                        break
                break
    ...
    for i, ar in enumerate(artranges1):  # Loop over each article in text1
        ...
        ar.append(
            "All"
        )  # Save indicator that this is all the changes (or matches) for this particular article
        allwords = [row[0] for row in ar[2]]
        c = collections.Counter(
            allwords
        )  # With last row, count the number of matches, additions etc. Save them with the lines below
        ar.append(c["match"])
...
def wheredidtheygo(ar, artnames2):
    ...
    for i, article in enumerate(
        artnames2
    ):  # Loop over the articles in the second text. Remember, this function is called article-by-article from text1.
        article = [
            artnames2[i],
            [],
        ]  # Make it easier to store list of matches, additions ets
        for step in ar[
            2
        ]:  # Loop over each step of the backtrace belonging in this article (in text1)
            if (
                re.sub(r"\n|\s", "", str(step[1])).upper()
                == re.sub(r"\n|\s", "", str(article[0])).upper()
            ):  # If the backtrace belongs in this article in the second text...
                article[1].append(
                    step[0]
                )  # ... Store that this is where the match, addition etc took place in the second text.
        article[1] = collections.Counter(
            article[1]
        )  # Reduce the backtrace to the number of matches, additions etc and store that instead
        if (
            len(article[1]) != 0
        ):  # If none of the backtrace steps ended up in this article in text 2, sort them away, store the rest.
            artnames_out.append(article)
    return artnames_out


def article_data_sorter_reverse(
    backtrace,
    artindex1,
    artindex2,
    lentext1,
    ...
    for step in backtrace:  # Loop over each step in the backtrace
        ...
        for j, ar in enumerate(artranges2):
            if (
                ar[1] <= rowindex and rowindex <= ar[0]
            ):  # With last row, find whic article the step belongs in (text1)
                for k, ar2 in enumerate(artranges1):
                    if (
                        ar2[1] <= columnindex and columnindex <= ar2[0]
                    ):  # Same for text 2
                        ar[2].append(
                            [step[2], artnames2[k]]
                        )  # Sort the step into a specific article and save it there
    ...
    for i, ar in enumerate(artranges2):  # Loop over each article in text1
        ...
        if (
            destination == "yes"
        ):  # If we also want to save if sections of the article have been moved to other articles
            wtdg = wheredidtheygo(
                ar, artnames1
            )  # Identify whether parts of article was moved to other articles and if so which.
            for line in wtdg:  # Save this information into the output
                new_lines.append(
                    [
                        caseid,
...
def artindexer(
    textlist,
):  # This finds the index position of every incidence of the word "YODABABY"
    text = textlist
    artindex = []
...
def artranger(artindex, lentext):
    ...
    if len(artindex) == 0:
        ...
        for i, index in enumerate(artindex):  # Loop through the article break points
            if (
                i == 0
            ):  # Special for the first break point, establish lowest bound of first article (recitals)
                lower = 0
            else:
            ...
            artranges.append(
                [lower, upper, []]
            )  # Save bounds to establish an index range of the article
    del artindex
...
def backtrace_line_matrix(lentext1, lentext2, ROWbacktrace):
    matrix = [
        [20] * (lentext2 + 1) for i in range(lentext1 + 1)
    ]  # Create nested list ~ a matrix with height of number of words in first text and width of number of words in second text
...
def backtrace_writer(
    inputs, text1, text2
):  # (inputs, text1, text2, outpath, propname, legname, caseid)
    context = [
        [
            "Text 1 index",
            "Text 2 index",
            "Edit operation",
            "Removed or substituted word",
            "Added or substituted word",
            "Word in both texts",
        ]
    ]
    for item in inputs:
    ...
    return context
    # with open(
    #     outpath + "backtraces/" + caseid + "-backtrace.csv", "wb"
    # ) as g:  # Save results
...
def plotter(
    ...
    if plottype == "bar":
        ...
        my_cmap, my_norm = matplotlib.colors.from_levels_and_colors(
            [-1, 4, 7, 12, 15, 19, 21],
            ["green", "red", "orange", "yellow", "blue", "white"],
        )  # Designate colors by setting bins of values (first list) and colors (second list)
        pylab.pcolormesh(
            plotarray, cmap=my_cmap, norm=my_norm
        )  # Fill plot with values, i.e. colors
        ...
        p1 = matplotlib.pyplot.Rectangle(
            (0, 0), 1, 1, fc="green"
        )  # 5 Legend boxes wth their color codes
        ...
        lgd = ax.legend(
            [p1, p2, p3, p4, p5],
            ["Unchanged", "Deletion", "Addition", "Substitution", "Transposition"],
            bbox_to_anchor=(1, -0.5),
            ncol=2,
        )  # Add legend ... loc='lower center',

    if plottype == "block":
        fig, ax = matplotlib.pyplot.subplots(
            1, figsize=(10, (10 * float(lentext1) / lentext2))
        )  # Set up plot
        my_cmap, my_norm = matplotlib.colors.from_levels_and_colors(
            [-1, 4, 7, 12, 15, 19, 21],
            ["green", "red", "orange", "yellow", "blue", "white"],
        ...
        lgd = ax.legend(
            [p1, p2, p3, p4],
            ["Unchanged", "Deletion", "Addition", "Substitution"],
            loc="upper center",
            bbox_to_anchor=(0.5, -0.1),
            ncol=2,
        )  # Add legend
        if by_article == "article":  # If an article-by-article plot is requested...
            text1artranges = artranger(
                text1artindex, lentext1
            )  # Find the article break points and names
            text2artranges = artranger(text2artindex, lentext2)
            for i, art in enumerate(text2artranges):
                matplotlib.pyplot.axvline(
                    art[1] + 1, color="grey", linestyle="-", linewidth=1
                )  # Plot line at every article break point
                matplotlib.pyplot.annotate(
                    artnames2[i],
                    (numpy.mean([float(art[0]), art[1]]), lentext1 / 25),
                    fontsize="small",
                    ha="center",
                    va="center",
                )  # Plot name of every article, does not work very well when loads of articles because of space limitations
            for j, art in enumerate(text1artranges):  # Same but for y-axis
                ...
                matplotlib.pyplot.annotate(
                    artnames1[j],
                    (lentext2 / 25, numpy.mean([float(art[0]), art[1]])),
                    rotation=90,
                    fontsize="small",
                    ha="center",
                    va="center",
    ...
    if plottype == "line":
        ...
        my_cmap, my_norm = matplotlib.colors.from_levels_and_colors(
            [-1, 4, 7, 12, 15, 19, 21],
            ["black", "#8B0000", "#D2691E", "#FF8C00", "#F0E68C", "white"],
        )  # Designate colors by setting bins of values (first list) and colors (second list)
        pylab.pcolormesh(
            plotarray, cmap=my_cmap, norm=my_norm
        )  # Fill plot with values, i.e. colors
    ...
    pylab.savefig(
        outpath + "plots/" + propname + "-" + legname + "-" + plottype + ".png",
...
def superloop(mat, cutoff):
    ...
    while row >= 1:  # Loop, weeeeeee!
        rowcontents = list(
            mat[row, :]
        )  # Store row contents in a nice, simple list format, rather than numpy array
        maxrowindex_found = 0  # Set up loop
        maxno = -1  # Set up loop
        while maxrowindex_found == 0:  # Start loop to find the best match on the row
            best_max = sorted(set(rowcontents))[
                maxno
            ]  # Identify the best match, the second best, etc, depending on value of maxno. First iteration will find best match, then maxno decreases and it finds second best match...
            best_max_index = [
                k for k, j in enumerate(rowcontents) if j == best_max
            ]  # Identify index of best match(es)
            if best_max == 0:  # If there is no (valid) match...
                rowmax = 0  # ... the rowmax is zero
                maxrowindex_found = (
                    1  # We can't find a better (or worse) match than that, job done.
                )
            else:  # If there are potentially valid matches
                for item in best_max_index:  # Loop through potential matches
                    if item not in collist and (
                        best_max > cutoff or item == lastrowmaxindex - 1
                    ):
                        if best_max == numpy.amax(
                            mat[:, item]
                        ):  # If the match is also the highest value on the column...
            ...
            maxno = (
                maxno - 1
            )  # If none of the potential matches where valid, ie where also highest value in their respective columns -> subtract 1 from maxno to try "worse" but potentially valid matches

        if rowmax > 0:  # If there is a match
            ROWbacktrace.append(
                [rowmaxindex, row, "match", rowmax]
            )  # Save coordinates and that there was a match
    ...
    while row < lastrow and column < lastcol:  # Loop over the columns
        if row + 1 not in rowlist:
            if column + 1 not in collist and column + 1 <= lastcol:
                ROWbacktrace.append(
                    [column + 1, row + 1, "substitution", 0]
                )  # ... then we are dealing with a substitution
                collist.append(
                    column + 1
                )  # Save backtrace coorindates for the cell diagonally down from the last cell
            ...
            if column + 1 not in collist:
                ROWbacktrace.append(
                    [column + 1, row, "addition", 0]
                )  # Then save the rowmaxindex in the backtrace coordinates
...
def text_compare(text1, text2):
    matrix2 = [
        [0] * (len(text2) + 1) for i in range(len(text1) + 1)
    ]  # Create nested list ~ a matrix with height of number of words in first text and width of number of words in second text
    matrix = numpy.array(
        matrix2
    )  # This can be edited out, but apparently is more memory efficient (if I am using it right!)
    del matrix2
    for i, word1 in enumerate(text1):  # Loop over each word in text1
        for j, word2 in enumerate(text2):  # Compare to each word in text2
            if word1 != word2:  # If they do not match...
                ...
                matrix[i + 1][j + 1] = (
                    matrix[i][j] + 1
                )  # Assign a value equal to the last word pair plus 1
    return matrix
...
def docu_toads_similarity(a, b):
    len_a = len(a)
    len_b = len(b)
    matrix = text_compare(a, b)
    output = superloop(matrix, 10)
    coutput = collections.Counter(list(zip(*output[0]))[2])
    total = sum([penalties[k] * v for k, v in coutput.items()])
    normalized = 1 - (total / (max(len_a, len_b) * 2))  # 2 is the maximum penalty
    return normalized
